...
// 默认符号表
// 正常应使用 SymbolManager::Instance() 进行动态加载
const std::unordered_map<std::string_view, int> g_Symbols = {
    {"!", 0},        {",", 1},          {"-", 2},      {".", 3},
    {"?", 4},        {"AA", 5},         {"AA0", 6},    {"AA1", 7},
...
bool InitSymbolsFromConfig(const std::string& config_path, const std::string& version) {
  auto& mgr = SymbolManager::Instance();

  // 如果未指定版本，尝试从config中读取
  std::string ver = version;
  if (ver.empty()) {
    // 尝试从config.json读取version字段
    std::ifstream file(config_path);
    if (file.is_open()) {
      try {
        nlohmann::json config = nlohmann::json::parse(file);
        if (config.contains("model") && config["model"].contains("version")) {
          ver = config["model"]["version"].get<std::string>();
        } else if (config.contains("version")) {
          ver = config["version"].get<std::string>();
        }
      } catch (...) {
        // 解析失败，使用默认版本
      }
  ...
  bool success = mgr.LoadFromConfig(config_path, ver);
  if (success) {
    mgr.SetActiveVersion(ver);
  }
  return success;
}

void SetActiveSymbolVersion(const std::string& version) {
  SymbolManager::Instance().SetActiveVersion(version);
}

std::string GetActiveSymbolVersion() {
  return SymbolManager::Instance().GetActiveVersion();
}

int FindSymbolId(const std::string& symbol) {
  return SymbolManager::Instance().FindSymbolActive(symbol);
}

bool HasSymbol(const std::string& symbol) {
  return SymbolManager::Instance().HasSymbolActive(symbol);
}
