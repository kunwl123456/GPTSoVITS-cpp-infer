...
static void PrintUsage(const char* prog) {
  std::cout << "用法: " << prog
            << " [speaker_package] [text] [lang] [speaker_name] [output]\n";
}
...
int main(int argc, char* argv[]) {
  ...
  std::string text = argc >= 3
                         ? argv[2]
                         : ("皆さん、我在インターネット上看到someone把几国langu"
                            "age混在一起speak。\n"
                            "虽然是混乱している句子ですけど、中文日本語プラスEn"
                            "glish、挑戦スタート！\n"
  ...
  try {
    ...
    auto audio = pipeline.Infer(
        speaker_name, text, text_lang, {}, 0.5f, 1.0f, &stats, [&]() {
    ...
    std::cout << "GPT Semantic Gen:       " << std::fixed
              << std::setprecision(3) << stats.gpt_time_s << "s"
              << " (" << std::fixed << std::setprecision(2)
              << stats.TokensPerSec() << " tokens/s,"
              << " " << stats.gpt_tokens << " tokens)\n";
    std::cout << "SoVITS Audio Decode:    " << std::fixed
              << std::setprecision(3) << stats.sovits_time_s << "s\n";
    if (first_segment_latency_ms > 0.0)
      std::cout << "First Segment Latency:  " << std::fixed
                << std::setprecision(2) << first_segment_latency_ms << " ms\n";
    std::cout << "Total Inference Time:   " << std::fixed
              << std::setprecision(3) << elapsed_ms / 1000.0 << "s\n";
    std::cout << "Total Audio Duration:   " << std::fixed
              << std::setprecision(3) << audio_duration << "s\n";
    std::cout << "Real Time Factor (RTF): " << std::fixed
